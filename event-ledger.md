...
Tests must be runnable with a standard command (e.g., `mvn test`).
...
- **Language:** Java
- **Database:** Each service uses its own in-memory or embedded database SQLite
- **Communication:** Synchronous REST calls between services
- **Tracing:** OpenTelemetry preferred
